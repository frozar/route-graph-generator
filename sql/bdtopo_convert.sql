...
DROP SERVER IF EXISTS bdtopo_server CASCADE;
CREATE SERVER bdtopo_server
  FOREIGN DATA WRAPPER postgres_fdw
  OPTIONS (host %(bdhost)s, port %(bdport)s, dbname %(dbname)s);

CREATE USER MAPPING FOR {user}
  SERVER bdtopo_server
...
CREATE TEMP TABLE IF NOT EXISTS bduni_troncon AS
  SELECT DISTINCT ON (cleabs) * FROM (
    SELECT
      ...
      -- ST_Force2D(ST_Transform(ST_SetSrid(t.geometrie, 2975), 4326)) as geom
      ST_Transform(ST_SetSrid(t.geometrie, 2975), 4326) as geom
...
WITH t AS (
  ...
  SELECT ST_StartPoint(geom) as geom FROM bduni_troncon
    UNION
  -- SELECT ST_Force2D(ST_EndPoint(geom)) as geom FROM bduni_troncon
  SELECT ST_EndPoint(geom) as geom FROM bduni_troncon
)
INSERT INTO nodes (lon,lat,alt,geom)
  -- SELECT ST_X(t.geom),ST_Y(t.geom),t.geom FROM t WHERE nodes_id(t.geom) IS NULL
  SELECT ST_X(t.geom),ST_Y(t.geom),ST_Z(t.geom),t.geom FROM t WHERE nodes_id(t.geom) IS NULL
;
...
INSERT INTO edges
  SELECT
    ...
    ST_Z(ST_StartPoint(geom)) as z1,
    ST_X(ST_EndPoint(geom)) as x2,
    ST_Y(ST_EndPoint(geom)) as y2,
    ST_Z(ST_EndPoint(geom)) as z2,
    -- ST_length(geography(ST_Transform(geom, 4326))) as length_m,
    ST_3Dlength(ST_Transform(geom, 4326)) as length_m,
    (CASE
